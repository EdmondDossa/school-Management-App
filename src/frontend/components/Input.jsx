...
const Input = ({ type, name, placeholder, value, onChange, className, previewFile=null }) => {
    ...
    if (type === "file") return (
        <div className="space-y-2">
            <div className={`${preview ? 'justify-between' : 'justify-end'} h-[50px] w-full flex  items-center bg-[#2871FA1A] rounded-md overflow-hidden`}>
                {preview && (
            ...
            {preview && (
                <p className="text-sm text-gray-500 truncate">
                    Fichier sélectionné: {value?.name || document.getElementById('fileInput')?.files[0]?.name}
                </p>
            )}
        </div>
    );
