...
class CoefficientService {
    static async getAllCoefficients() {
        ...
        return rows.map(row => new Coefficient(row.Cod_Mat, row.Coef, row.Annee, row.Num_Class));
    }

    static async getCoefficientByCodMat(codMat, Annee, Num_Class) {
        const sql = "SELECT * FROM coefficientsMatieres WHERE Cod_Mat = ?, Annee = ?, Num_Class = ?";
        const rows = await window.electronAPI.db.query(sql, [codMat, Annee, Num_Class]);
        if (rows.length === 0) return null;
        const row = rows[0];
        return new Coefficient(row.Cod_Mat, row.Coef, row.Annee, row.Num_Class);
    }
