...
class InscriptionService {
    static async getAllInscriptions() {
        ...
        return rows.map(row => new Inscription(row.Num_Ins, row.Date_Ins, row.Statut, row.Matricule, row.Num_Class, row.Annee_Scolaire));
    }
    ...
    static async getInscriptionByNum(numIns) {
        ...
        const row = rows[0];
        return new Inscription(row.Num_Ins, row.Date_Ins, row.Statut, row.Matricule, row.Num_Class, row.Annee_Scolaire);
    }
    ...
    static async createInscription(inscription) {
        const sql = "INSERT INTO inscriptions (Date_Ins, Statut, Matricule, Num_Class, Annee_Scolaire) VALUES (?, ?, ?, ?, ?)";
        const result = await window.electronAPI.db.query(sql, [inscription.dateIns, inscription.statut, inscription.matricule, inscription.numClass, inscription.anneeScolaire]);
        return result;
    }
    ...
    static async updateInscription(inscription) {
        const sql = "UPDATE inscriptions SET Date_Ins = ?, Statut = ?, Matricule = ?, Num_Class = ?, Annee_Scolaire = ? WHERE Num_Ins = ?";
        const result = await window.electronAPI.db.query(sql, [inscription.dateIns, inscription.statut, inscription.matricule, inscription.numClass, inscription.anneeScolaire, inscription.numIns]);
        return result;
    }
